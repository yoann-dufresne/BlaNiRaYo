# started coding it but never finished!
...
if do_mip:
    ...
    T = list(range(nb_days//5)) # average number of time periods (signups are between 1 and 10)
    w = [libs[i].signup for i in L]

    print("subscription times", w[:10])

    m = Model('knap solver 2e')
    #m.verbose=0

    l = [[m.add_var(var_type=BINARY) for t in T] for i in L] # XXXXXXXXXXX the gist is here: libs are (lib_id, day) / uncontinued after that idea.. 
    ...
    for li in L: #FIXME: hmm comment gerer le fait qu'on veut selectionner des livres pas deja pris dans d'autres libs..
